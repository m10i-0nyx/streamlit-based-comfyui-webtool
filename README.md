...
### ✨ プロンプト入力支援機能（NEW）
- **Danbooruタグ辞書**: [Hugging Face](https://huggingface.co/datasets/newtextdoc1111/danbooru-tag-csv)から自動取得
- **多言語検索**: 英語・日本語でタグを検索可能
- **人気タグ表示**: 使用頻度の高いタグをワンクリックで追加
- **ネガティブプロンプトプリセット**: 品質改善やNSFW回避などのテンプレート
- 詳細は [docs/PROMPT_HELPER.md](docs/PROMPT_HELPER.md) を参照
...
- 各出力画像にダウンロードボタンを表示
- 1セッションあたり同時リクエスト数を環境変数で制限。完了後は追加リクエスト可能
